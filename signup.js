// Küçük koruma: bazı overlay/eklenti katmanlarını periyodik gizle
(function guard(){
  try {
    document.body.contentEditable = "false"; document.designMode = "off";
    const kill = (k)=>document.querySelectorAll([id*="${k}"],[class*="${k}"]).forEach(el=>el.remove());
...
// Sekmeler
$("tabLogin").addEventListener("click", ()=>{ clr(); $("tabLogin").classList.add("active"); $("tabSignup").classList.remove("active"); show("loginView"); hide("signupView"); });
$("tabSignup").addEventListener("click",()=>{ clr(); $("tabSignup").classList.add("active"); $("tabLogin").classList.remove("active"); show("signupView"); hide("loginView"); });

// ENTER ile gönder (klik bloklansa bile)
